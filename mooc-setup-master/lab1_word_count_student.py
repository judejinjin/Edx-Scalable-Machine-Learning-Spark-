...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda (x,iterator) : (x,sum(iterator)))
...
totalCount = wordCounts.values().reduce(add)
...
def removePunctuation(text):
    ...
    """
    words = map(lambda s: s.strip(), text.lower().split())
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = wordCount(shakeWordsRDD).takeOrdered(15, lambda (word,count): -count) 
