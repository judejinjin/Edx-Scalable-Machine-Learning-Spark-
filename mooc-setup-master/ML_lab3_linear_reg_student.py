...
def parsePoint(line):
    ...
    """
    words = line.split(',')
    return LabeledPoint(words[0], map(float, words[1:]))
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData, numIters, 1, 1, None, reg, regType, True)
...
def twoWayInteractions(lp):
    ...
    """
    pairs = list(itertools.product(lp.features,repeat=2))
    pairs = map(lambda x:x[0]*x[1], pairs)
    
    return LabeledPoint(lp.label, np.hstack((lp.features, pairs)))
...
modelInteract = LinearRegressionWithSGD.train(trainDataInteract, numIters, alpha,
                                              miniBatchFrac, regParam=reg,
                                              regType='l2', intercept=True)
labelsAndPredsInteract = trainDataInteract.map(lambda lp: (lp.label, modelInteract.predict(lp.features)))
